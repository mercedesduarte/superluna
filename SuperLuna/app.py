...
def update_category(category_id):
    try:
        data = request.get_json()
        print(f"✏️ Actualizando categoría {category_id}:", data)
...
def delete_category(category_id):
    try:
        print(f"🗑️ Eliminando categoría {category_id}")
        
        categoria = Category.query.get(category_id)
        if not categoria:
            return jsonify({'success': False, 'message': 'Categoría no encontrado'}), 404
    ...
    try:
        ...
        return jsonify({
            ...
            'product': {
                ...
                'discount': nuevo_producto.discount,
                'icon': nuevo_producto.icon,
                'stock': nuevo_producto.stock,
                'is_daily_offer': nuevo_producto.is_daily_offer
            }
        }), 201
...
def update_product(product_id):
    try:
        data = request.get_json()
        print(f"✏️ Actualizando producto {product_id}:", data)
...
def delete_product(product_id):
    try:
        print(f"🗑️ Eliminando producto {product_id}")
